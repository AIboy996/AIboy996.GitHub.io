...
最近断断续续听了647老师(B站UP我真的不懂分析)的复分析, 受益匪浅. 本系列笔记, 总结一些我认为复分析中有意思的内容.
...
我在网上找到了一份多伦多大学的Geoffrey Scott同学写的[代数笔记](https://www.math.toronto.edu/gscott/oct28_2015.pdf)，相关部分截图如下（这里面**constructible**就是**规矩的**的意思）：
...
<s>这也就证明了，这个数不能在实数范围内用有限次的加减乘除开方表示。</s>

!!! warning "勘误"
    这只能证明，这个数不能在实数范围内用有限次的加减乘除和**开二次方**表示（换言之，这个数不是一个[规矩数](../algebra/constructible_number.md)）。正确的证明需要Galois群的相关知识。请看另外一篇文章：[不可约情形](../algebra/casus_irreducibilis.md)。
